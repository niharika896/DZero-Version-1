...
void parseCastlingRights(const string &fenCastling, GameState &s)
{
    s.WhiteCanCastleKingSide = 0;
    s.WhiteCanCastleQueenSide = 0;
    s.BlackCanCastleKingSide = 0;
    s.BlackCanCastleQueenSide = 0;
    ...
    for (char c : fenCastling)
    {
        ...
        {
        case 'K':
            s.WhiteCanCastleKingSide = 1;
            break;
        case 'Q':
            s.WhiteCanCastleQueenSide = 1;
            break;
        case 'k':
            s.BlackCanCastleKingSide = 1;
            break;
        case 'q':
            s.BlackCanCastleQueenSide = 1;
            break;
        }
    }
}
...
void printBoardWithPieces(const GameState &state)
{
    map<Piece, char> pieceSymbols = {
        {WHITE_PAWN, 'P'}, {WHITE_KNIGHT, 'N'}, {WHITE_BISHOP, 'B'}, {WHITE_ROOK, 'R'}, {WHITE_QUEEN, 'Q'}, {WHITE_KING, 'K'}, {BLACK_PAWN, 'p'}, {BLACK_KNIGHT, 'n'}, {BLACK_BISHOP, 'b'}, {BLACK_ROOK, 'r'}, {BLACK_QUEEN, 'q'}, {BLACK_KING, 'k'}};
...
void printMovesWithNames(const vector<pair<Piece, vector<Move>>> &moves)
{
    map<Piece, string> pieceNames = {
        {WHITE_PAWN, "White Pawn"}, {WHITE_KNIGHT, "White Knight"}, {WHITE_BISHOP, "White Bishop"}, {WHITE_ROOK, "White Rook"}, {WHITE_QUEEN, "White Queen"}, {WHITE_KING, "White King"}, {BLACK_PAWN, "Black Pawn"}, {BLACK_KNIGHT, "Black Knight"}, {BLACK_BISHOP, "Black Bishop"}, {BLACK_ROOK, "Black Rook"}, {BLACK_QUEEN, "Black Queen"}, {BLACK_KING, "Black King"}};
...
void updateCombinedBoards(GameState &s)
{
    ...
    for (int p = WHITE_PAWN; p <= WHITE_KING; ++p)
        whiteAll |= s.board.pieces[p];
    for (int p = BLACK_PAWN; p <= BLACK_KING; ++p)
        blackAll |= s.board.pieces[p];
...
public:
    ...
    ParsedFEN parseFEN(const string &fen, Helper &fenHelper)
    {
        vector<string> parts;
        string cur;
        ...
        // 2 = castling
        // 3 = ep target
        fenHelper.loadFEN(parts[0], out.board);
        out.sideToMove = (parts[1] == "w" ? WHITE : BLACK);
        out.castling = parts[2];
        out.enpassant = parts[3];
    ...
    bool isSquareAttacked(const GameState &s, Color attacker, int sq)
    {
        Bitboard occ = s.board.pieces[ALL_PIECES];
        Bitboard target = 1ULL << sq;

        // ---------------- Pawn ----------------
        if (attacker == WHITE)
        {
            Bitboard wp = s.board.pieces[WHITE_PAWN];
            Bitboard attacks =
                ((wp & ~FILE_H) << 7) |
                ((wp & ~FILE_A) << 9);
            if (attacks & target)
        ...
        {
            Bitboard bp = s.board.pieces[BLACK_PAWN];
            Bitboard attacks =
                ((bp & ~FILE_A) >> 7) |
                ((bp & ~FILE_H) >> 9);
            if (attacks & target)
        ...
        while (nn)
        {
            int from = __builtin_ctzll(nn);
            nn &= nn - 1;
            if (knightMoves[from] & target)
                return true;
        }
        ...
        while (kk)
        {
            int from = __builtin_ctzll(kk);
            kk &= kk - 1;
            if (kingMoves[from] & target)
                return true;
        }
        ...
        while (bb)
        {
            ...
            bb &= bb - 1;
            if (Magic.getBishopAttacks(from, occ) & target)
                return true;
        }
        ...
        while (rr)
        {
            int from = __builtin_ctzll(rr);
            rr &= rr - 1;
            if (Magic.getRookAttacks(from, occ) & target)
                return true;
        }
    ...
    void makeMove(GameState &s, const Move &m)
    {
        ...
        if ((mover == WHITE_PAWN || mover == BLACK_PAWN) && s.enPassantSquare != -1 && to == s.enPassantSquare)
        {
            if (mover == WHITE_PAWN)
            {
                int capSq = to - 8;
                if (capSq >= 0)
                    s.board.pieces[BLACK_PAWN] &= ~(1ULL << capSq);
                s.enPassantSquare = -1;
            }
            else
            {
                int capSq = to + 8;
                if (capSq < 64)
                    s.board.pieces[WHITE_PAWN] &= ~(1ULL << capSq);
                s.enPassantSquare = -1;
            }
        }
            ...
            while (bb)
            {
                ...
                if (p == (state.sideToMove == WHITE ? WHITE_KING : BLACK_KING))
                {
                    if (state.sideToMove == WHITE)
                    {
                        ...
                        if (state.WhiteCanCastleKingSide && (state.board.pieces[WHITE_ROOK] & (1ULL << 7)) && (state.board.pieces[WHITE_KING] & (1ULL << 4)))
                        {
                            if (!(occupancy & ((1ULL << 5) | (1ULL << 6))))
                            {
                                ...
                                if (eSafe && fSafe && gSafe)
                                    movesForPiece.push_back(Move{4, 6, NO_PIECE});
                            }
                        }
                        // white queen-side e1->c1 (4->2)
                        if (state.WhiteCanCastleQueenSide && (state.board.pieces[WHITE_ROOK] & (1ULL << 0)) && (state.board.pieces[WHITE_KING] & (1ULL << 4)))
                        {
                            if (!(occupancy & ((1ULL << 1) | (1ULL << 2) | (1ULL << 3))))
                            {
                                ...
                                if (eSafe && dSafe && cSafe)
                                    movesForPiece.push_back(Move{4, 2, NO_PIECE});
                            }
                        }
                    }
                    else
                    {
                        ...
                        if (state.BlackCanCastleKingSide && (state.board.pieces[BLACK_ROOK] & (1ULL << 63)) && (state.board.pieces[BLACK_KING] & (1ULL << 60)))
                        {
                            if (!(occupancy & ((1ULL << 61) | (1ULL << 62))))
                            {
                                ...
                                if (eSafe && fSafe && gSafe)
                                    movesForPiece.push_back(Move{60, 62, NO_PIECE});
                            }
                        }
                        // black queen-side e8->c8 (60->58)
                        if (state.BlackCanCastleQueenSide && (state.board.pieces[BLACK_ROOK] & (1ULL << 56)) && (state.board.pieces[BLACK_KING] & (1ULL << 60)))
                        {
                            if (!(occupancy & ((1ULL << 57) | (1ULL << 58) | (1ULL << 59))))
                            {
                                ...
                                if (eSafe && dSafe && cSafe)
                                    movesForPiece.push_back(Move{60, 58, NO_PIECE});
                            }
                        }
                    }
                }
...
void enforceCastlingLegality(Game &g, GameState &s)
{
    // Helper lambdas for occupancy/attacks checks
    auto occupied = [&](int sq)
    { return (s.board.pieces[ALL_PIECES] & (1ULL << sq)) != 0; };
    auto attacked = [&](Color attacker, int sq)
    { return g.isSquareAttacked(const_cast<GameState &>(s), attacker, sq); };

    // --- WHITE KING SIDE (e1 king, h1 rook) ---
    if (s.WhiteCanCastleKingSide)
    {
        // must have king on e1 and rook on h1
        if (!(s.board.pieces[WHITE_KING] & (1ULL << 4)) || !(s.board.pieces[WHITE_ROOK] & (1ULL << 7)))
        {
            s.WhiteCanCastleKingSide = 0;
        }
        else
        {
            // squares between e1 and h1 (f1=5,g1=6) must be empty
            if (occupied(5) || occupied(6))
            {
                s.WhiteCanCastleKingSide = 0;
            }
            else
            {
                // king must not be in check, and f1/g1 must not be attacked by black
                if (attacked(BLACK, 4) || attacked(BLACK, 5) || attacked(BLACK, 6))
                {
                    s.WhiteCanCastleKingSide = 0;
                }
            }
    ...
    // --- WHITE QUEEN SIDE (e1 king, a1 rook) ---
    if (s.WhiteCanCastleQueenSide)
    {
        if (!(s.board.pieces[WHITE_KING] & (1ULL << 4)) || !(s.board.pieces[WHITE_ROOK] & (1ULL << 0)))
        {
            s.WhiteCanCastleQueenSide = 0;
        }
        else
        {
            // squares between a1 and e1: b1=1,c1=2,d1=3 must be empty
            ...
            {
                // king must not be in check, and d1/c1 must not be attacked by black
                if (attacked(BLACK, 4) || attacked(BLACK, 3) || attacked(BLACK, 2))
                {
                    s.WhiteCanCastleQueenSide = 0;
                }
            }
    ...
    // --- BLACK KING SIDE (e8 king=60, h8 rook=63) ---
    if (s.BlackCanCastleKingSide)
    {
        if (!(s.board.pieces[BLACK_KING] & (1ULL << 60)) || !(s.board.pieces[BLACK_ROOK] & (1ULL << 63)))
        {
            s.BlackCanCastleKingSide = 0;
        }
        else
        {
            if (occupied(61) || occupied(62))
            {
                s.BlackCanCastleKingSide = 0;
            }
            else
            {
                if (attacked(WHITE, 60) || attacked(WHITE, 61) || attacked(WHITE, 62))
                {
                    s.BlackCanCastleKingSide = 0;
                }
            }
    ...
    // --- BLACK QUEEN SIDE (e8 king=60, a8 rook=56) ---
    if (s.BlackCanCastleQueenSide)
    {
        if (!(s.board.pieces[BLACK_KING] & (1ULL << 60)) || !(s.board.pieces[BLACK_ROOK] & (1ULL << 56)))
        {
            s.BlackCanCastleQueenSide = 0;
        }
        else
        {
            if (occupied(57) || occupied(58) || occupied(59))
            {
                s.BlackCanCastleQueenSide = 0;
            }
            else
            {
                if (attacked(WHITE, 60) || attacked(WHITE, 59) || attacked(WHITE, 58))
                {
                    s.BlackCanCastleQueenSide = 0;
                }
            }
